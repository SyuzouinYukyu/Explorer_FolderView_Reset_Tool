...
1. `Explorer_FolderView_Reset_Tool_v1.0.0.exe` を起動します。
2. 必要に応じて `BagMRU Size` とバックアップ先を変更します。
3. `事前チェック` で対象キー、現在値、Explorer起動状態、バックアップ先を確認します。
4. `バックアップのみ実行` で `.reg` バックアップだけを作成できます。
5. `修復を実行` でバックアップ、Explorer停止、対象キー削除、`BagMRU Size` 設定、Explorer再起動を実行します。
6. 処理後、任意のフォルダで表示形式を変更し、Explorerを閉じて再度開き、表示形式が保存されるか確認します。
...
- Windows再起動は原則不要ですが、改善しない場合はサインアウト、またはWindows再起動を推奨します。
...
現時点では LICENSE ファイルを追加していません。公開前にライセンスを選択してください。
